...
% rajoutter encodage des entités nommés avec des Entity taggers
...
Le chapitre suivant décrit le workflow de travail mis en place pour le format pivot TEI. La figure \ref{fig:workflow_tei_pivot_lectaurep} ci-contre, reprend les différentes étapes qui seront présentées.
\begin{figure}[h]
    \centering
    \centerline{\fbox{\includegraphics[width=7cm]{workflow_tei_pivot_lectaurep}}}
...
\begin{itemize}
    \item La balise \citecode{<teiHeader>}\footnote{\cite{tei_tei_nodate}} pour encoder la partie la plus théorique, correspondant aux métadonnées relatives aux archives physiques et aux images (données issues des fichiers XML EAD/EAC et EXIF) ;
...
Pour générer l'ODD spécifique du projet nous avons utiliser une feuille de style \citecode{oddbyexample.xsl} fourni par la TEI pour générer la documentation à partir du canevas du format pivot XML TEI, c'est-à-dire la source \citecode{template\_pivot\_TEI\_lectaurep.xml}, dans les formats suivants : XML (format natif), HTML, pour une visualisation web, et PDF\footnote{L'ODD du projet (dans les formats cités) est disponible dans les Annexes, \citecode{/B-Format\_pivot\_XML\_TEI\_Lectaurep/Doc/Modélisation\_et\_documentation\_format\_pivot/}}.\\
...
\end{itemize}
\newpage
\section{Simuler l'agrégation des données et la validation d'un fichier XML TEI via un \textit{script} Python}
...
\begin{enumerate}
    ...
    \item Le script principal charge et stocke distinctement les différents fichiers à partir de l'extension de ces derniers (le script fait un appel au module \citecode{extract\_utils.py} via le \textit{package built-in} glob qui gère la recherche de chemins de style Unix);
    ...
    \item L'étape suivante consiste à extraire des fichiers, dans les catalogues XML, les données sont placées dans deux nouvelles arborescence XML distinctes qui correspondent aux informations des fichiers XML ALTO placés dans une première structure arborescente réparti avec un niveau \citecode{<facsimile>} et un niveau \citecode{<text>} et les informations des XML ALTO disposés dans un \citecode{<teiHeader>} dans une seconde structure arborescente. Les structures sont créés à partir des feuilles XSL, qui sont lues par le pré-processeur XSL java Saxon. Ce dernier est appelé par une ligne de commande qui s'exécute automatiquement durant le programme par l'intermédiaire d'une fonction du \textit{package} os qui permet d'interagir avec le système d'exploitation;
    \item Les deux structures sont alors rassemblés (\textit{merge}) en une seule qui comprend désormais le \citecode{<teiHeader>}, le \citecode{<facsimile>} et le \citecode{<text>};
    \item Les métadonnées EXIF sont extraites des images et rassembler dans une arborescence \citecode{<xenoData>} qui rejoint l'arborescence précédente de manière à constituer le fichier XML TEI complet. A noter que durant cette étape certaines valeurs EXIF n'ont pas été décodé au moment de leurs insertions dans l'arborescence, nous avons donc ajouter une valeur par défaut \inquote{bytes values} pour ces valeurs qui ne sont pas prises en compte\footnote{une balise \citecode{<exif>} doit obligatoirement contenir une valeur, dans le canevas XML TEI j'ai ajouté au niveau du \citecode{<encodingDesc>} cette information de manière à ce que l'on puisse interpréter cette information ultérieurement.};
