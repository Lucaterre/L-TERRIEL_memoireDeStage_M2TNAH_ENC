...
%Ce mémoire est réalisé dans le cadre de l'obtention du diplôme du Master \inquote{Technologies numériques appliquées à l'histoire} de l'École nationale des chartes. Il fait suite à un stage d'application de quatre mois à INRIA, au sein de l'équipe projet ALMAnaCH, en soutien au projet Lectaurep. 
%Son propos sera principalement orienté sur le développement d'un format pivot XML TEI ayant pour but de représenter et d'homogénéiser les données et métadonnées, présentent dans la chaîne de traitement et n'étant actuellement pas interopérables. Il traitera également de la question de l'évaluation des modèles de transcriptions, entraînés avec Kraken grâce à la conception \textit{ex-nihilo} d'une application web en Python.
%Nous nous concentrerons notamment sur la phase 3 de ce projet, dans la mesure où celui-ci cours sur plusieurs années.
%Il met également en avant les liens entretenus par les différents acteurs du projet, ainsi qu'entre les deux principales missions du stage : [explain en 4 mots].
%Il nous permettra d'envisager les perspectives d'optimisation dans le développement des outils présentés.\\
