  ...
  -->
    ...
    <xsl:template match="/">
        <xsl:variable name="file">
            <xsl:value-of select="lower-case(/eadid)"/>
        </xsl:variable>
            ...
            <xsl:result-document href="{concat($file, '.xml')}">
                <teiHeader xmlns="http://www.tei-c.org/ns/1.0">
                    <fileDesc>
                        <titleStmt>
                            <title>
                                <xsl:value-of
                                    select="normalize-space(//titleproper)"/>
                            </title>
                            <subtitle>
                                <xsl:value-of
                                    select="normalize-space(//titlestmt/subtitle)"
                                />
                            </subtitle>
                        </titleStmt>
                        <publicationStmt>
                            <p>XML/TEI Working file for a Lectaurep project which consists of a chain template to retrieve technical metadatas (EXIF,XMP,IPTC) from images, IR EAD and EAC and allow interoperability with the IIIF protocol</p>
                        </publicationStmt>
                        <xsl:if test="//archdesc/scopecontent/p">
                            <notesStmt>
        ...
        <xsl:variable name="mainLang">
            <xsl:call-template name="guessLang">
                <xsl:with-param name="langtext">
                    <xsl:value-of select="normalize-space(lower-case(@langmaterial))"/>
                </xsl:with-param>
            </xsl:call-template>
        </xsl:variable>
        <msContents>
